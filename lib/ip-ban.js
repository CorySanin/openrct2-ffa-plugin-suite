(function () {
    ...
    function main() {
        ...
        if (network.mode === 'server') {
            var LOG_IN_CHAT_1 = context.sharedStorage.get('ip-ban.log-in-chat', true);
                ...
                if (command !== false) {
                    if ((args = doesCommandMatch(command, [CMDUNBAN])) !== false) {
                        ...
                        if (isPlayerAdmin(getPlayer(e.player)) && args.length > 0) {
                            ...
                            outmsg = "{YELLOW}Banned ".concat(match ? match[0] : args[0], " for ").concat(t, " minutes!");
                        }
                    }
                    if (outmsg) {
            ...
            context.registerAction(ACTION_NAME, function (arg) {
                ...
                var rawStat = arg.args.stat;
                var stat = rawStat.replace('n-', '');
                if (bannedObjects[stat] in bannedIPs && !(ip in bannedIPs)) {
                    ...
                    var ipStr = LOG_IN_CHAT_1 ? "(".concat(ip, ")") : '..';
                    bannedObjects[stat] = ip;
                    network.kickPlayer(player.id);
                    if (timeout_1 > 0) {
                        sendToAdmins("Kicked ".concat(player.name, " ").concat(ipStr, ". Time remaining: ").concat(Math.ceil((timeout_1 - date.ticksElapsed) / TICKS_PER_MINUTE), " minutes."));
                    }
                    else {
                        sendToAdmins("Kicked ".concat(player.name, " ").concat(ipStr, "."));
                    }
                }
                else {
                    if (LOG_IN_CHAT_1) {
                        sendToAdmins("".concat(player.name, "'s key: ").concat(rawStat));
                    }
