        ...
        if (network.mode === 'server') {
            ...
            context.registerAction(ACTION_NAME, function (arg) {
                ...
                if (bannedObjects[stat] in bannedIPs && !(ip in bannedIPs)) {
                    ...
                    if (timeout_1 > 0) {
                        sendToAdmins("Kicked ".concat(player.name, " (").concat(ip, "). Time remaining: ").concat(Math.ceil((timeout_1 - date.ticksElapsed) / TICKS_PER_MINUTE), " minutes."));
                    }
                    else {
                        sendToAdmins("Kicked ".concat(player.name, " (").concat(ip, ")."));
                    }
                }
                else {
